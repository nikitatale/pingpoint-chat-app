    ...
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-slate-600'>Expand your circle, meet like-minded people, and turn every connection into a meaningful relationship.</p>
        </div>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {
            dataArray.find((item) => item.label === currentTab).value.map((user) => (
              <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                <img src={user.profile__picture} className='rounded-full w-12 h-12 shadow-md mx-auto' alt="image" />
                <div className='flex-1'>
                   <p className='font-medium text-slate-700'>{user.full_name}</p>
                   <p className='text-slate-500'>@{user.username}</p>
                   <p className='text-slate-500 text-sm'>{user.bio.slice(0, 30)}...</p>
                   <div className='flex max-sm:flex-col gap-2 mt-4'> 
                     {
                        <button onClick={() => navigate(`/profile/${user._id}`)} className='w-full p-2 text-sm rounded bg-gradient-to-r from-[#111B3D] to-[#394b88] hover:from-[#394b88] hover:to-[#111B3D] active:scale-95 transition text-white cursor-pointer'>
                      View Profile
                      </button>
                     }
                     {
