...
const StoryViewer = ({viewStory, setViewStory}) => {
  ...
  const renderContent = () => {
    switch (viewStory.media_type) {
        ...
        case 'video':
            return (
                <video controls autoPlay src={viewStory.media_url} className='max-h-screen' onEnded={() => setViewStory(null)}/>
            );    
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify-center' style={{backgroundColor: viewStory.media_type === 'text' ? viewStory.background_color: '#000000'}}>
        ...
        <div className='absolute top-4 left-4 flex items-center space-x-3 p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
         <img src={viewStory.user?.profile__picture} alt="image" className='size-7 sm:size-8 rounded-full object-cover border border-white' />
        ...
        <button className='absolute top-4 right-4 text-white text-3xl font-bold focus:outline-none' onClick={handleClose}>
            <X className='w-8 h-8 hover:scale-100 transition cursor-pointer'/>
        </button>
